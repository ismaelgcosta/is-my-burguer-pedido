...
create table pedido (valor_total numeric(38,2), cliente_id uuid, pedido_id uuid not null, status_pedido varchar(255) check (status_pedido in ('ABERTO','FECHADO','PAGO','AGUARDANDO_PAGAMENTO','AGUARDANDO_CONFIRMACAO_PAGAMENTO','PAGAMENTO_NAO_AUTORIZADO','RECEBIDO','EM_PREPARACAO','PRONTO','FINALIZADO')), primary key (pedido_id));
